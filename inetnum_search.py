...
def cidr_to_mask(cidr):
    return str(ipaddress.IPv4Network(cidr).netmask)
...
def main(search_term):
    ...
    for (start, end), mnt_by in process_file(search_term):
        count += 1
        try:
            cidr = ipaddress.summarize_address_range(ipaddress.IPv4Address(start), ipaddress.IPv4Address(end))
            for network in cidr:
                print(f"{network}\t{network.network_address}\t{cidr_to_mask(str(network))}")
    ...
    if count == 0:
        ...
        print(f"Found {count} matching ranges.")
