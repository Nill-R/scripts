...
# Original: https://github.com/nixcraft/domain-check-2
#
# Author: Matty < matty91 at gmail dot com >
# Forked by: Nill Ringil <nill.ringil at proton dot me>
#
...
check_domain_status() {
	...
	elif [ "${TLDTYPE}" == "edu" ]; then # added by nixCraft 26-aug-2017
		REGISTRAR=$(cat ${WHOIS_TMP} | ${AWK} -F: '/Registrant:/ && $0 != ""  { getline;REGISTRAR=substr($0,1,17) } END { print REGISTRAR }')
	elif [ "${TLDTYPE}" == "cafe" ]; then # added by @kode29 26-aug-2017
		REGISTRAR=$(cat ${WHOIS_TMP} | ${AWK} -F: '/Registrar:/ && $0 != "" { REGISTRAR=substr($0,12,17) } END { print REGISTRAR }')

	elif [ "${TLDTYPE}" == "link" ]; then # added by @kode29 26-aug-2017
		REGISTRAR=$(cat ${WHOIS_TMP} | ${AWK} -F: '/Registrar:/ && $0 != "" {  REGISTRAR=substr($0,12,17) } END { print REGISTRAR }')
	elif [ "${TLDTYPE}" == "blog" ]; then # added by @kode29 26-aug-2017
		REGISTRAR=$(cat ${WHOIS_TMP} | ${AWK} -F: '/Registrar:/ && $0 != "" {  REGISTRAR=substr($0,12,17) } END { print REGISTRAR }')
	elif [ "${TLDTYPE}" == "ru" -o "${TLDTYPE}" == "su" ]; then # added 20141113
		REGISTRAR=$(cat ${WHOIS_TMP} | ${AWK} -F: '/registrar:/ && $2 != "" { REGISTRAR=substr($2,6,17) } END { print REGISTRAR }')
	elif [ "${TLDTYPE}" == "cz" ]; then # added by Minitram 20170830
	...
	if [ "${TLDTYPE}" == "info" -o "${TLDTYPE}" == "org" ]; then
		...
		*) tmonth=0 ;;
		...
		tday=$(echo ${tdomdate} | ${CUT} -d "-" -f 3 | ${CUT} -d "T" -f 1)
		DOMAINDATE=$(echo "${tday}-${tmonth}-${tyear}")
	elif [ "${TLDTYPE}" == "com" -o "${TLDTYPE}" == "net" -o "${TLDTYPE}" == "org" -o "${TLDTYPE}" == "link" -o "${TLDTYPE}" == "blog" -o "${TLDTYPE}" == "cafe" -o "${TLDTYPE}" == "biz" -o "${TLDTYPE}" == "us" -o "${TLDTYPE}" == "mobi" -o "${TLDTYPE}" == "tv" -o "${TLDTYPE}" == "co" -o "${TLDTYPE}" == "pro" -o "${TLDTYPE}" == "cafe" -o "${TLDTYPE}" == "in" -o "${TLDTYPE}" == "cat" -o "${TLDTYPE}" == "asia" -o "${TLDTYPE}" == "cc" -o "${TLDTYPE}" == "college" -o "${TLDTYPE}" == "aero" ]; then # added on 26-aug-2017 by nixCraft
	...
	elif [ "${TLDTYPE}" == "mx" ]; then # added by nixCraft 07/jan/2019
		tdomdate=$(cat ${WHOIS_TMP} | ${AWK} '/Expiration Date:/ { print $3 }')
		tyear=$(echo ${tdomdate} | ${CUT} -d "-" -f 1)
		tmon=$(echo ${tdomdate} | ${CUT} -d "-" -f 2)
		case ${tmon} in
	...
	elif [ "${TLDTYPE}" == "it" ]; then # added by nixCraft 07/jan/2019 based upon https://github.com/pelligrag
	...
	elif [ "${TLDTYPE}" == "ro" ]; then # added by nixCraft 07/jan/2019
